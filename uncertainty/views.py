...
def index(request):
    ...
    if not request.user.is_authenticated():
        ...
        unique_directory_name = 'uid_' + str(request.user.id) + '_uncertainty_' + \
    ...
    if request.method == 'POST':
        if 'swat_model_zip' in request.FILES:
            ...
            if os.path.exists(loc):
                ...
                request.session['uncertainty_swat_model_dir'] = unique_path + '/input/' + \
                    swat_model_filename
...
def upload_landuse_zip(request):
    ...
    if request.method == 'POST':
        if 'landuse_zip' in request.FILES:
            ...
            request.session['uncertainty_landuse_dir'] = unique_path + '/input/' + \
                landuse_filename
            request.session['progress_message'].append(
...
def upload_lookup_file(request):
    ...
    if request.method == 'POST':
        ...
        if 'lookup_file' in request.FILES:
            ...
            request.session['uncertainty_lookup_filepath'] = unique_path + '/input/' + \
                lookup_filename
            request.session['progress_message'] = []
            ...
            context.push({
                'uncertainty_lookup_loop_times': [i + 1 for i in range(len(lookup_info))],
...
def update_error_percentage(request):
    ...
    if request.method == 'POST':
        ...
        request.session['progress_message'].append('Errors and realizations selected.')

    # Send loop times back in case someone wants to re-update their landuse percentage errors
    context = RequestContext(request)
    context.push({'uncertainty_lookup_loop_times': request.session.get('uncertainty_lookup_loop_times')})
...
def request_process(request):
    ...
    data = {
        ...
        'hrus1_dir': request.session.get('uncertainty_swat_model_dir') + '/Watershed/Grid/hrus1',
...
def download_data(request):
    ...
    if task_id != '':
        ...
        if int(user_id) == int(request.user.id):
            if os.path.exists(settings.BASE_DIR + '/user_data/' + request.user.email + '/' + task_id + '/output/Output'):
                file = io.BytesIO()
