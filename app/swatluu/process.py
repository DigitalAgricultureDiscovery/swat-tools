...
class SWATLUUProcess(object):
    ...
    def create_output_dir(self):
        ...
        if os.path.exists(output_dir):
            if os.path.exists(output_dir + '/Raster'):
                shutil.rmtree(output_dir + '/Raster')
            if os.path.exists(output_dir + '/Output'):
                shutil.rmtree(output_dir + '/Output')
            if os.path.exists(output_dir + '/Shape'):
                shutil.rmtree(output_dir + '/Shape')
            shutil.rmtree(output_dir)

        os.makedirs(output_dir + '/Raster')
        os.makedirs(output_dir + '/Output')
        os.makedirs(output_dir + '/Shape')

        shutil.copytree(self.hrus1_dir, output_dir + '/Raster/hrus1')
    ...
    def create_lupdat_file(self):
        ...
        try:
            lup_file = open(self.output_dir + '/lup.dat', 'a')
        except IOError as e:
            self.logger.error(str(e))
        ...
        for layer_index in range(0, len(self.landuse_layers_names)):
            ...
            converted_layer_path = self.output_dir + '/Raster/' + layer_name + '.tif'
            try:
    ...
    def merge_thresholds(self):
        ...
        try:
            geotools.create_raster(
                hrus,
                self.output_dir + '/Raster/hrus1/w001001.adf',
                self.output_dir + '/Raster/final_HRU.tif')
        except Exception as e:
            self.logger.error(str(e))
    ...
    def create_fractional_values(self):
        ...
        hru_areas_file = open(self.output_dir + '/hru_areas.txt', 'w')
        ...
        for landuse_layer_index, landuse_layer in enumerate(
            ...
            try:
                landuse_layer_raster, layer_info = geotools.create_raster_array(
                    self.output_dir + '/Raster/hrus1',
                    self.output_dir + '/Raster/' + landuse_layer[3] + '.tif')
            ...
            fractional_hru_areas_file = open(
                self.output_dir + '/Output/file' + str(
