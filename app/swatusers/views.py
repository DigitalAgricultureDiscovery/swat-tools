...
def delete_user_data(request):
    ...
    upload_path = os.path.join(settings.USER_UPLOAD_DIR, request.user.email, 'swatluu')

    if (upload_path is not None):
        # Delete user's uploaded input data
        if os.path.exists(os.path.join(upload_path, 'input')):
            shutil.rmtree(os.path.join(upload_path, 'input'))
        # Delete user's output data
        if os.path.exists(os.path.join(upload_path, 'output')):
            shutil.rmtree(os.path.join(upload_path, 'output'))
    ...
    if "HTTP_APITOKEN" in request.META:
        ...
        if apikey == settings.APIKEY:
            ...
            db = mydatabase.MyDatabase(
                mycnf=os.path.join(settings.PROJECT_DIR, "/swatapps/my.cnf"))
